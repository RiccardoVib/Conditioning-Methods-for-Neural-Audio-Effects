...
def train(**kwargs):
    ...
    if not inference:
        ...
        opt = tf.keras.optimizers.Adam(learning_rate=MyLRScheduler(learning_rate, training_steps, epochs1, 4), clipnorm=1)   
        losses = 'mse'
    ...
    with open(os.path.normpath('/'.join([model_save_dir, save_folder, str(model_name) + str(dataset) + 'results.txt'])), 'w') as f:
